...
1. Make the change following the rules in `.cursor/rules/`.
2. Verify in dev (`npm run dev`) and that `npm run build` passes.
3. After layout changes, run `npm run qa:viewport` (preview + Playwright).
4. Record user-visible changes in `CHANGELOG.md` under `## [Unreleased]` — delegate to the `changelog-keeper` agent or update it directly.
5. Use the `verifier` agent for QA on feature work before considering a task done.
...
1. Check `primal_concept.txt` for messaging
2. Check `DESIGN_SYSTEM.md` §14 for UI guardrails
3. Prefer smallest diff that satisfies the request
4. Do not invent new product domains or rename existing ones
